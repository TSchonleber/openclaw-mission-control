"""Telemetry helpers for Nara Hub backend."""
...
class TelemetryEvent:
    """Normalized event emitted by CodexRouter for telemetry tracking."""
...
class TelemetryTracker:
    """Tracks rolling latency/routing metrics and exposes snapshots for the UI."""
    ...
    def _latency_stats(self, samples: list[float]) -> Dict[str, float | None]:
        if not samples:
    ...
    def _error_stats(self, events: list[tuple[datetime, str, bool]]) -> Dict[str, float | int]:
        ...
        errors = sum(1 for _ts, _route, err in events if err)
